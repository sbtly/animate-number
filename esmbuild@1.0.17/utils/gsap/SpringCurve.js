// src/utils/gsap/SpringCurve.tsx
import { settlingDuration } from "./SettlingDurationSolver.js";
function getSpringEasing({ stiffness, damping }) {
...
var SpringCurve = class {
  ...
  getDamping(mass = 1) {
    return this.getDampingRatio() * 2 * Math.sqrt(this.getStiffness(mass) * mass);
  }
  getSettlingDuration() {
  ...
  constructor(p) {
    ...
    const damping = Math.max(0, p.damping);
    const response = 2 * Math.PI / Math.sqrt(stiffness / mass);
    const dampingRatio = Math.min(1, damping / 2 / Math.sqrt(stiffness * mass));
  ...
  curveFunc(t) {
    ...
    if (Math.abs(zeta - 1) < 1e-8) {
      ...
      y = Math.sqrt(1 + c2 * c2) * Math.exp(a * t) * Math.cos(b * t + theta + Math.PI);
    }
    return y + 1;
  }
  derivativeCurveFunc(t) {
    ...
    if (zeta === 1) {
      ...
      let c2 = (v0 + a) / b;
      let theta = Math.atan(c2);
      return Math.sqrt(1 + c2 * c2) * Math.exp(a * t) * (a * Math.cos(b * t + theta + Math.PI) - b * Math.sin(b * t + theta + Math.PI));
    }
  }
};
export {
  SpringCurve,
  getSpringEasing as default
};
