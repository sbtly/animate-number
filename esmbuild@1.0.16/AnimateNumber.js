...
function AnimateNumber(props) {
  ...
  const loopingDuration = (_a = props.loopingDuration) != null ? _a : 0.3;
  ...
  useEffect(() => {
    ...
    const toArrOnlyNum2 = getDigitsArray(props.to);
    console.log(isNaN(props.from), isNaN(props.to));
    console.log(props.from, props.to);
    console.log(fromArrOnlyNum, toArrOnlyNum2);
    setToArrOnlyNum(toArrOnlyNum2);
    setToIsLargerThenFrom(isToLargerThenFromWithQuestionmark(props.from, props.to));
    const fromArr = isNaN(props.from) ? convertStringToArray(props.from) : (_a2 = props.from) == null ? void 0 : _a2.toLocaleString().split("");
    const toArr = isNaN(props.to) ? convertStringToArray(props.to) : (_b2 = props.to) == null ? void 0 : _b2.toLocaleString().split("");
    console.log(fromArr, toArr);
    const toIsLongerThenFrom2 = fromArr.length === toArr.length ? isToLargerThenFromWithQuestionmark(props.from, props.to) : fromArr.length < toArr.length;
    setToIsLongerThenFrom(toIsLongerThenFrom2);
    console.log("isToLonger: ", toIsLongerThenFrom2, "isToLarger: ", isToLargerThenFromWithQuestionmark(props.from, props.to));
    let resultFromArr2 = [];
    let resultToArr2 = [];
    let resultFromArrOnlyNum2 = [];
    let resultToArrOnlyNum2 = [];
    resultFromArr2 = props.align === "right" ? getResultFromArrFromEnd(fromArr, toArr, props.rollAllDigits) : getResultFromArr(fromArr, toArr, props.rollAllDigits);
    resultToArr2 = props.align === "right" ? getResultToArrFromEnd(fromArr, toArr, props.rollAllDigits) : getResultToArr(fromArr, toArr, props.rollAllDigits);
    resultFromArrOnlyNum2 = props.align === "right" ? getResultFromArrFromEnd(fromArrOnlyNum, toArrOnlyNum2, props.rollAllDigits) : getResultFromArr(fromArrOnlyNum, toArrOnlyNum2, props.rollAllDigits);
    resultToArrOnlyNum2 = props.align === "right" ? getResultToArrFromEnd(fromArrOnlyNum, toArrOnlyNum2, props.rollAllDigits) : getResultToArr(fromArrOnlyNum, toArrOnlyNum2, props.rollAllDigits);
    ...
    console.log(resultFromArr2, resultToArr2);
    console.log(resultFromArrOnlyNum2, resultToArrOnlyNum2);
    const fromPositionArr2 = generatePositions(fromArr, numWidths);
    setFromPositionArr(fromPositionArr2);
    console.log("fromPositionArr: ", fromPositionArr2);
    const loadingPositionArr2 = generatePositions(toIsLongerThenFrom2 ? resultToArr2 : resultFromArr2, numWidths);
    setLoadingPositionArr(loadingPositionArr2);
    console.log("loadingPositionArr: ", loadingPositionArr2);
    const fromArrCommaIndices = findCommaIndices(fromArr);
    console.log("from comma ", fromArrCommaIndices);
    const toArrCommaIndices2 = findCommaIndices(toArr);
    console.log("to comma ", toArrCommaIndices2);
    setToArrCommaIndices(toArrCommaIndices2);
    const fromPositionArrOnlyNum2 = removeElementsByIndices(fromPositionArr2, fromArrCommaIndices);
    setFromPositionArrOnlyNum(fromPositionArrOnlyNum2);
    console.log("fromPositionArrOnlyNum: ", fromPositionArrOnlyNum2);
    const loadingPositionArrOnlyNum2 = removeElementsByIndices(loadingPositionArr2, toIsLongerThenFrom2 ? toArrCommaIndices2 : fromArrCommaIndices);
    setLoadingPositionArrOnlyNum(loadingPositionArrOnlyNum2);
    console.log("loadingPositionArrOnlyNum: ", loadingPositionArrOnlyNum2);
    const endPositionArr2 = generatePositions(toArr, numWidths);
    setEndPositionArr(endPositionArr2);
    console.log("endPositionArr: ", endPositionArr2);
    const endPositionArrOnlyNum2 = removeElementsByIndices(endPositionArr2, toArrCommaIndices2);
    setEndPositionArrOnlyNum(endPositionArrOnlyNum2);
    console.log("endPositionArrOnlyNum: ", endPositionArrOnlyNum2);
    const loopIndices2 = getIndicesOfLoops(toIsLongerThenFrom2 ? resultToArrOnlyNum2 : resultFromArrOnlyNum2, resultToShowLoops);
    setLoopIndices(loopIndices2);
    console.log(loopIndices2, toIsLongerThenFrom2);
    const loopStartNums = getStartNumsOfLoops(toIsLongerThenFrom2 ? resultToArrOnlyNum2 : resultFromArrOnlyNum2, fromArrOnlyNum, toArrOnlyNum2, toIsLargerThenFrom, props.align);
    setLoopArr(loopStartNums);
    console.log(loopStartNums, props.align);
    const suffixFromPos = calculateSuffixPosition(fromArr, numWidths, fromPositionArr2);
    setSuffixFromPosition(suffixFromPos);
    const suffixLoadingPos = calculateSuffixPosition(toIsLongerThenFrom2 ? resultToArr2 : resultFromArr2, numWidths, loadingPositionArr2);
    setSuffixLoadingPosition(suffixLoadingPos);
    const suffixEndPos = calculateSuffixPosition(toArr, numWidths, endPositionArr2);
  ...
  useLayoutEffect(() => {
    let ctx = gsap.context(() => {
      ...
      props.align !== "right" && gsap.set(suffixRef.current, { x: suffixFromPosition });
      tl.current = gsap.timeline({
        delay: props.delay
      });
      toIsLargerThenFrom ? outFromCharsMotion.current = gsap.to(outFromChars, __spreadValues({
        rotateX: 60,
        y: "-50%",
        opacity: 0,
        stagger: {
      ...
      }, spring.small)) : props.quickMode ? outFromCharsMotion.current = gsap.to(outFromChars, __spreadValues({
        rotateX: -60,
        y: "50%",
        opacity: 0,
        stagger: {
          ...
          each: function(index, target, list) {
            return resultFromArrOnlyNum[index] === "hide" ? 0 : index * (props.quickMode ? 0.03 : outStaggerDelay);
          }
        }
      }, spring.quick)) : outFromCharsMotion.current = gsap.to(outFromChars, __spreadValues({
        rotateX: -60,
        y: "50%",
        opacity: 0,
        stagger: {
      ...
      outFromCommaMotion.current = gsap.to(outFromCommas, __spreadValues({
        opacity: 0
      }, spring.rapid));
      repositionLoopsMotion.current = gsap.to(q(`.${loopContainerStyle}`), {
        x: function(i) {
          return toIsLongerThenFrom ? endPositionArrOnlyNum[loopIndices[i]] : fromPositionArrOnlyNum[loopIndices[i]];
        },
        duration: 0
      });
      showLoopsMotion.current = gsap.fromTo(q(`.${loopContainerStyle}`), {
        opacity: 0
      }, __spreadValues({
        opacity: 1,
        stagger: {
      ...
      showToCommaMotion.current = gsap.fromTo(inToCommas, {
        opacity: 0
      }, __spreadValues({
        opacity: 1,
        stagger: function(i) {
          return props.align === "right" ? (endPositionArr.length - 1 - toArrCommaIndices[i]) * (props.quickMode ? 0.03 : inStaggerDelay) : toArrCommaIndices[i] * (props.quickMode ? 0.03 : inStaggerDelay);
        }
      }, expandEasing));
      toIsLongerThenFrom ? hideLoopsMotion.current = gsap.to(q(`.${loopContainerStyle}`), __spreadValues({
        opacity: 0,
        stagger: {
          from: props.align === "right" ? "end" : 0,
          each: inStaggerDelay
        }
      }, hideLoopEasing)) : hideLoopsMotion.current = gsap.to(q(`.${loopContainerStyle}`), __spreadValues({
        opacity: 0,
        stagger: {
          from: props.align === "right" ? "end" : 0,
          each: function(index, target, list) {
            return resultFromArrOnlyNum[index] === "hide" ? 0 : index * inStaggerDelay;
          }
        }
      }, hideLoopEasing));
      repositionFromSameCharsMotion.current = gsap.to(fromSplitCharsOnlyNum, __spreadValues({
        x: function(i) {
          if (props.align === "right") {
            ...
            const map = fromReversed == null ? void 0 : fromReversed.map((n, i2) => {
              return endReversed[i2] - n + widthDiff;
            });
            const mapReversed = map == null ? void 0 : map.toReversed();
      ...
      repositionToCommaMotion.current = gsap.to(inToCommas, __spreadValues({
        x: 0
      }, expandEasing));
      toIsLargerThenFrom ? showToCharsMotion.current = gsap.fromTo(inToChars, { rotateX: -50, opacity: 0, y: "50%" }, __spreadValues({
        rotateX: 0,
        opacity: 1,
        y: "0%",
        stagger: {
      ...
      }, inEasing)) : showToCharsMotion.current = gsap.fromTo(inToChars, { rotateX: 50, opacity: 0, y: "-50%" }, __spreadValues({
        rotateX: 0,
        opacity: 1,
        y: "0%",
        stagger: {
      ...
      if (props.hidePreSuffixWhileMoving) {
        if (props.align !== "right") {
          hidePreSuffixMotion.current = gsap.to(q(`.${suffix}`), __spreadValues({
            opacity: 0
          }, spring.rapid));
          showPreSuffixMotion.current = gsap.fromTo(q(`.${suffix}`), {
            x: toIsLongerThenFrom ? suffixEndPosition - 6 : suffixEndPosition + 6
          }, __spreadValues({
            opacity: 1,
            x: suffixEndPosition,
          ...
          hidePreSuffixMotion.current = gsap.to(q(`.${prefix}`), __spreadValues({
            opacity: 0
          }, spring.rapid));
          showPreSuffixMotion.current = gsap.to(q(`.${prefix}`), __spreadValues({
            opacity: 1
          }, spring.rapid));
        }
      } else {
      ...
      !props.hidePreSuffixWhileMoving && props.align !== "right" ? moveSuffixToEndMotion.current = gsap.to(q(`.${suffix}`), __spreadValues({
        x: suffixEndPosition
      }, toIsLongerThenFrom ? expandEasing : shrinkEasing)) : moveSuffixToEndMotion.current = null;
      !props.hidePreSuffixWhileMoving && props.align === "right" ? toIsLongerThenFrom ? movePrefixToEndMotion.current = gsap.from(q(`.${prefix}`), __spreadValues({
        x: toValWidth - fromValWidth,
        immediateRender: false
      }, expandEasing)) : movePrefixToEndMotion.current = gsap.to(q(`.${prefix}`), __spreadValues({
        x: fromValWidth - toValWidth
      }, shrinkEasing)) : movePrefixToEndMotion.current = null;
      if (props.align === "center") {
        toIsLongerThenFrom ? changeWidthMotion.current = gsap.to(wrapperRef.current, {
        ...
        toIsLongerThenFrom ? repositionAllByAlignMotion.current = gsap.to(wrapperRef.current, __spreadValues({
          x: 0
        }, expandEasing)) : repositionAllByAlignMotion.current = gsap.to(wrapperRef.current, __spreadValues({
          x: (fromValWidth - toValWidth) / 2
        }, shrinkEasing));
      } else if (props.align === "right") {
        repositionAllByAlignMotion.current = null;
        !toIsLongerThenFrom ? resetPrefixToEndMotion.current = gsap.to(q(`.${prefix}`), {
          x: 0,
          duration: 1e-3
      ...
      if (props.quickMode) {
        toIsLongerThenFrom ? (_a2 = tl.current) == null ? void 0 : _a2.addLabel("outFrom").add(changeWidthMotion.current, "outFrom").add(repositionAllByAlignMotion.current, "outFrom").add(outFromCharsMotion.current, "outFrom").add(outFromCommaMotion.current, "outFrom").add(repositionFromSameCharsMotion.current, "outFrom").add(repositionToCommaMotion.current, "outFrom").add(moveSuffixToEndMotion.current, "outFrom").add(movePrefixToEndMotion.current, "outFrom").addLabel("showTo", "outFrom+=0.05").add(showToCommaMotion.current, "showTo").add(showToCharsMotion.current, "showTo") : (_b2 = tl.current) == null ? void 0 : _b2.addLabel("outFrom").add(repositionAllByAlignMotion.current, "outFrom").add(outFromCharsMotion.current, "outFrom").add(outFromCommaMotion.current, "outFrom").addLabel("showTo", "outFrom+=0.05").add(showToCommaMotion.current, "showTo").add(repositionFromSameCharsMotion.current, "showTo").add(repositionToCommaMotion.current, "showTo").add(moveSuffixToEndMotion.current, "showTo").add(movePrefixToEndMotion.current, "showTo").add(showToCharsMotion.current, "showTo").addLabel("endMotion").add(changeWidthMotion.current, "endMotion").add(resetPrefixToEndMotion.current, "endMotion");
      } else {
        toIsLongerThenFrom ? (_c2 = tl.current) == null ? void 0 : _c2.addLabel("outFrom").add(changeWidthMotion.current, "outFrom").add(repositionAllByAlignMotion.current, "outFrom").add(outFromCharsMotion.current, "outFrom").add(outFromCommaMotion.current, "outFrom").add(hidePreSuffixMotion.current, "outFrom").addLabel("repositionAllDigits", "outFrom").add(repositionFromSameCharsMotion.current, "outFrom").add(repositionToCommaMotion.current, "outFrom").add(moveSuffixToEndMotion.current, "outFrom").add(movePrefixToEndMotion.current, "outFrom").add(repositionLoopsMotion.current, "repositionAllDigits").add(showLoopsMotion.current, "repositionAllDigits").addLabel("showComma", "repositionAllDigits").add(showToCommaMotion.current, "showComma").addLabel("endLooping", `outFrom+=${loopingDuration}`).addLabel("repositionToEnd", "endLooping").addLabel("hideLoop", "endLooping").addLabel("showTo", "endLooping").add(hideLoopsMotion.current, "hideLoop").add(hideLoopsGroupMotion.current, "hideLoop").add(showToCharsMotion.current, "showTo").add(showPreSuffixMotion.current, "showTo") : (_d = tl.current) == null ? void 0 : _d.addLabel("outFrom").add(outFromCharsMotion.current, "outFrom").addLabel("repositionAllDigits", "outFrom").add(repositionLoopsMotion.current, "repositionAllDigits").add(showLoopsMotion.current, "repositionAllDigits").addLabel("endLooping", `outFrom+=${loopingDuration}`).addLabel("repositionToEnd", "endLooping").addLabel("hideLoop", "endLooping").addLabel("showTo", "endLooping").add(outFromCommaMotion.current, "endLooping").add(hidePreSuffixMotion.current, "endLooping").add(showToCommaMotion.current, "endLooping").add(hideLoopsMotion.current, "hideLoop").add(hideLoopsGroupMotion.current, "hideLoop").add(repositionAllByAlignMotion.current, "repositionToEnd").add(repositionFromSameCharsMotion.current, "repositionToEnd").add(repositionToCommaMotion.current, "repositionToEnd").add(moveSuffixToEndMotion.current, "repositionToEnd").add(movePrefixToEndMotion.current, "repositionToEnd").add(showToCharsMotion.current, "showTo").add(showPreSuffixMotion.current, "showTo+=0.1").addLabel("endMotion").add(changeWidthMotion.current, "endMotion").add(resetPrefixToEndMotion.current, "endMotion");
      }
    });
    return () => {
      ctx.revert();
  ...
  }, props.from && ((_b = props.from) == null ? void 0 : _b.toLocaleString())), /* @__PURE__ */ React.createElement("div", {
  ...
  }, !props.quickMode && loopArr.map((n, i) => {
    return /* @__PURE__ */ React.createElement(Loop, {
      key: i,
      i,
      play: playLoop,
      ...
      startStaggerDelay: props.align === "right" ? outStaggerDelay * (loopArr.length - 1 - i) : outStaggerDelay * i,
      toIsLargerThenFrom
    });
  })), /* @__PURE__ */ React.createElement("div", {
    className: toNum,
    style: { left: props.align === "right" ? "unset" : 0 }
  }, props.to && ((_c = props.to) == null ? void 0 : _c.toLocaleString()))), /* @__PURE__ */ React.createElement("div", {
    ref: suffixRef,
    className: suffix
  }, props.suffix)));
}
