...
// src/utils/gsap/gsap.tsx
import getSpringEasing from "./SpringCurve.js";
import { ixdEasing } from "./token.js";
gsapWithCSS.registerPlugin(CustomEase, ScrollTrigger2);
...
var { easingFn: slowEase, duration: slowDuration } = getSpringEasing(ixdEasing.spring.slow);
var { easingFn: slow2Ease, duration: slow2Duration } = getSpringEasing(ixdEasing.spring.slow2);
var { easingFn: basicEase, duration: basicDuration } = getSpringEasing(ixdEasing.spring.basic);
var { easingFn: basic2Ease, duration: basic2Duration } = getSpringEasing(ixdEasing.spring.basic2);
var { easingFn: bounce2Ease, duration: bounce2Duration } = getSpringEasing(ixdEasing.spring.bounce2);
var { easingFn: bounce3Ease, duration: bounce3Duration } = getSpringEasing(ixdEasing.spring.bounce3);
var { easingFn: smallEase, duration: smallDuration } = getSpringEasing(ixdEasing.spring.small);
var { easingFn: mediumEase, duration: mediumDuration } = getSpringEasing(ixdEasing.spring.medium);
var { easingFn: largeEase, duration: largeDuration } = getSpringEasing(ixdEasing.spring.large);
var { easingFn: quickEase, duration: quickDuration } = getSpringEasing(ixdEasing.spring.quick);
var { easingFn: rapidEase, duration: rapidDuration } = getSpringEasing(ixdEasing.spring.rapid);
var springs = {
  slow: {
    ease: slowEase,
...
gsapWithCSS.registerEffect({
  name: "addSpace",
  effect: (targets) => gsapWithCSS.to(gsapWithCSS.effects.addSpace, { duration: targets[0] }),
  extendTimeline: true
});
export {
  Back,
